...
func parseConfigKeys(path string) ([]configKeySpec, error) {
	...
	descByKey := map[string]string{
		...
		"ci_remote":                    "Git remote to query for CI and PR/MR status. When unset, an `upstream` remote is preferred when present, otherwise `origin`. Set to a remote name (e.g. `origin`) to override. Useful for fork workflows where pull requests live on the upstream repository.",
