# How does it compare?

See [COMPARISON.md](https://github.com/chmouel/lazyworktree/blob/main/COMPARISON.md) for a detailed comparison with other worktree managers.

## Trivia

Originally a Python [Textual](https://textual.textualize.io/) app, migrated to Go ([BubbleTea](https://github.com/charmbracelet/bubbletea)) for faster startup. Python version: <https://github.com/chmouel/lazyworktree/tree/python>

## Copyright

[Apache-2.0](https://github.com/chmouel/lazyworktree/blob/main/LICENSE)

## Authors

### Chmouel Boudjnah

* 🐘 Fediverse - <[@chmouel@chmouel.com](https://fosstodon.org/@chmouel)>
* 🐦 Twitter - <[@chmouel](https://twitter.com/chmouel)>
* 📝 Blog  - <[https://blog.chmouel.com](https://blog.chmouel.com)>

## Next Steps

<div class="mint-card-grid">
  <a class="mint-card" href="index.md">
    <strong>Introduction</strong>
    <span>Return to documentation entry points and learning paths.</span>
  </a>
  <a class="mint-card" href="troubleshooting/faq.md">
    <strong>FAQ</strong>
    <span>Find concise answers to common practical questions.</span>
  </a>
</div>
