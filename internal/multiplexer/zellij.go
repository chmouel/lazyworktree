...
func BuildZellijScript(sessionName string, zellijCfg *config.TmuxCommand, layoutPaths []string) string {
	...
	b.WriteString("  zellij list-sessions --no-formatting 2>/dev/null | grep -v EXITED | sed 's/ \\[.*//' | grep -Fxq \"$1\"\n")
	b.WriteString("}\n")
	...
	b.WriteString("  while ! zellij list-sessions --no-formatting 2>/dev/null | grep -v EXITED | sed 's/ \\[.*//' | grep -Fxq \"$session\"; do\n")
